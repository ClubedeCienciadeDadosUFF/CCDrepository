...
extract_data <- function(first_element = 1,
  ...
  if (repair) {
    ...
    numbers_fixed <- c()
    for (file in files) {
      numbers_fixed <- c(numbers_fixed, as.vector(unique(read.csv(file)[, 2])))
  ...
  while (ifelse(last_element == "ALL", 
    ...
    result <- tryCatch( {
        ...
        if (repair) {
          while (n == numbers_fixed[1] && !identical(numbers_fixed, integer(0))) {
              if (n < last_element) {
                n <- n + 1                        
              }
      ...
      if (grepl("UNHANDLED_ERROR", result)) {
        dataset_log <- rbind(dataset_log, data.frame(LOG = paste(n, " [UNHANDLED_ERROR] ", result, 
                                                                 " TimeStamp: ", Sys.time()),
      ...
      dataset_log <- rbind(dataset_log, data.frame(LOG = paste("occurrence ", n, " read.",
                                                               " TimeStamp: ",Sys.time()),
                                                   stringsAsFactors = FALSE))
