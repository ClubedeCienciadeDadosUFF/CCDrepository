...
extract_data_parallel <- function(first_element = 1,
                                 last_element  = "ALL", 
                                 repair        = FALSE, 
                                 update        = FALSE, 
                                 sample        = 1000) {
    ...
    #
    # Strong Points:
    #   - Explicit parallelism (via parallel built-in package)
    ...
    # Setup a cluster to use all logical cores
    write("", EXECUTION_LOG)
    cluster <- makeCluster(detectCores())
    registerDoParallel(cluster, cores = detectCores())
    ...
    while (ifelse(last_element == "ALL", 
        ...
        # Parallelism: %dopar%; Sequential: %do%
        dataset <- foreach(i = n:(n + sample - 1), 
                        .packages = c('XML'), .combine = rbind) %dopar% {
            result <- tryCatch( {
                ...
                sink(EXECUTION_LOG, append = TRUE)         
                print(paste("occurrence ", i, " read\n"))
                sink()
                
                # organize the data retrieved
                return(data.frame(n_occurrence, latitude, longitude, 
