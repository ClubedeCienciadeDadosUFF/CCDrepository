...
add_tree(_,Tree,Predict):-
	...
	p_message('best clause'),%debug, spy pp_clause/1,
	pp_dclause(Clause1),
        nlits(Clause,L),
	Gain is -Cost,
...
find_clause(Search):-
	...
	p_message('best clause'),%debug, spy pp_dclause/1,
	pp_dclause(RClause),
	show_stats(Evalfn,BestLabel),
...
soldnf_solve(Goal,Status,SkippedSoFar,SkippedSoFar):-
	soldnf_builtin(Goal), !, write(Status),nl,
...
soldnf_clause(Goal,Body):-write(Goal), %nl,(Goal = rdiff(_27336,3,_27745) -> debug, spy clause/2; true),write(Goal),nl,
	clause(Goal,Body).
